...
Game.prototype = {
	init: function init() {
		...
		if( !$('.title').length ) 
		$(this.container).append($(document.createElement("div")).text("WP3").addClass("title"));
...
startMenu: function startMenu(back) {
		...
		$(this.content).append(this.createButton((this.save.savegame.start == null ? "Start Game" : "Continue"), this.newGame));
		//$(this.content).append(this.createButton(this.lang.current.menu.lang, this.language));
		//$(this.content).append(this.createButton("Trigger Warning", this.triggerWarning));
		$(this.content).append(this.createButton("Credits", this.credits));
		$(this.content).append(this.createButton("Reset", this.resets));
		...
		//Create the "Back toHypnotic Owl" link
	...
	//This is where the send statistics was
